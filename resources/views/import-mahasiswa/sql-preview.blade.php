...
@section('title', 'SQL Preview — Import Mahasiswa')

@section('content')
<section class="section">
    <div class="section-header">
        <h1>Preview Import dari DB Lama</h1>
        <div class="section-header-breadcrumb">
            <div class="breadcrumb-item"><a href="{{ route('home') }}">Dashboard</a></div>
            <div class="breadcrumb-item"><a href="{{ route('import-mahasiswa.index') }}">Import</a></div>
            <div class="breadcrumb-item active">SQL Preview</div>
        </div>
    </div>
    ...
    <div class="section-body">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h4 class="mb-0"><i class="fas fa-database mr-2 text-primary"></i> Data dari DB Lama</h4>
                <div>
                    ...
                    <table class="table table-hover mb-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $row['email'] }}</td>
                                <td>{{ $row['npm'] }}</td>
                                <td>{{ $row['gender'] === 'L' ? 'L' : ($row['gender'] === 'P' ? 'P' : '-') }}</td>
                                <td>{{ $row['old_prodi'] ? 'Prodi #' . $row['old_prodi'] : '-' }}</td>
                                <td>
                                    @if($row['exists'] === 'Baru')
                            ...
                            @empty
                            <tr><td colspan="7" class="text-center py-4 text-muted">Database lama kosong atau tidak dapat diakses.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
        ...
        <div class="card">
            <div class="card-body">
                <form action="{{ route('import-mahasiswa.sql-run') }}" method="POST">
                    @csrf
                    <input type="hidden" name="gelombang_id" value="{{ $gelombang->id }}">
                    <input type="hidden" name="data" value="{{ json_encode(array_values(array_filter($rows, fn($r) => $r['exists'] === 'Baru'))) }}">
                    ...
                    <div class="d-flex gap-2">
                        <button type="submit" class="btn btn-success" onclick="return confirm('Import {{ $newRows }} mahasiswa dari DB lama sekarang?')">
